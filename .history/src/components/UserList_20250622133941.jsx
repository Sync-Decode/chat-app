...
export default function UserList() {
  ...
  const handleUpsertChat = async (user) => {
    ...
    // Step 1: Get all chat_ids for otherUser
    ...
    if (otherUserChatsError) {
      console.error(
        'Error fetching other user chats:',
        otherUserChatsError.message
      )
      return
    }

    const otherUserChatIds = otherUserChats.map((item) => item.chat_id)

    // Step 2: Find shared chat with currentUser where chat.type === 'dm'
    ...
    // Step 4: Add both users to chat_members
